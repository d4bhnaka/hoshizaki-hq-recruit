...
| M-3 | 本文＋サブビジュアル | `.p-message__body` | 左：本文（4 段落の縦長テキスト、サンプル文言は [画像 2 参照]）／右：社員 3 人談笑写真＋フードサービス機器写真（上下 2 枚組）／**デザインメモ「人の写真は使わない／採用ペンギン」が Figma に付箋として残存**：最終デザインでは人物写真をペンギンイラストに差し替える予定の可能性あり → **実装前に確認** |
...
| フードサービス機器写真 | JPG | `public/assets/message-photo-02.jpg` |
...
| # | セクション | クラス | 主な中身 |
|:--|:--|:--|:--|
| S-1 | `PageHeader` | — | 英字 `Beyond HOSHIZAKI` ／サブ「ホシザキの事業戦略」／薄青空背景 |
...
| S-7 | Global Pioneers（Life Beyond Borders） | `.p-strategy-pioneers` | 見出し英字 `Life Beyond Borders`／サブ「国境を越えて、働く。暮らす。」／導入文／**駐在社員メッセージカード ×3**（フィリピン・マニラ／英国・テルフォード／米国・ジョージア州）。各カードは六角エリア名バッジ＋**駐在社員プロフィール**（→「S-7 駐在社員プロフィール」）＋トピック（見出し＋本文、一部は右に写真サムネイル）。Figma node `836:2191` |
| S-8 | CTA ペア | — | `BottomCta`（INTERNSHIP / ENTRY） |
| S-9 | パンくず | — | `Breadcrumb`：`採用TOP ▸ ホシザキの海外展開` |
...
### S-7 駐在社員プロフィール（エリア名バッジ直下）

各カードのエリア名バッジ直下に、社員紹介ページの `__meta` と同形式（`.p-strategy-pioneers__meta`・縦積み・二次テキスト色／イニシャルのみ太字）でプロフィールを表示する。データは各 pioneer の `profile` オブジェクト（`strategy.astro`）が単一ソース。空文字のフィールドは非表示。

表示順（クライアント指定の 4 項目）：**イニシャル → 事業部名 → 出向先企業名 → 入社年　卒業学部学科名**（末尾行は入社年と学部学科を全角スペースで 1 行に連結）。

| エリア | イニシャル | 事業部名 | 出向先企業名 | 入社年 | 卒業学部学科名 |
|:--|:--|:--|:--|:--|:--|
| フィリピン・マニラ | S.A | 海外事業部 | Hoshizaki Philippines Corporation出向 | 2024年入社 | 経済学部卒 |
| 英国・テルフォード | S.Y | 技術部 | HOSHIZAKI EUROPE LIMITED出向 | 2009年入社 | 電気工学科卒 |
| 米国・ジョージア州 | M.R | 経理部 | Hoshizaki America, Inc.出向 | 2019年入社 | 経済学部経済学科卒 |

出典：クライアント支給「海外出向者 アンケートシート」。サイト規約に合わせ、入社年は「◯年入社」（佐藤さんの「2009年度」→「2009年入社」）、学部学科は末尾に「卒」を付与（佐藤さんはアンケートに学科のみ＝「電気工学科卒」）。村瀬さんの出向先社名はアンケート未記載のため「HA役職」＋ピーチツリーシティ本社の記述から **Hoshizaki America, Inc.** と補完（要最終確認）。アンケートの役職・肩書（Sales & Marketing／Assistant Manager）は 4 項目に含まれないため非表示。
...
| J-3 | 導入 | `.p-job__intro` | 「ホシザキ株式会社（本社）には、製品を生み出す開発設計や生産技術、製品を世界へ広める営業企画や海外事業企画、会社全体を支えるコーポレートや社内SEなど、多様な職種のプロフェッショナルが集まっています。…部門を超えて連携する『Team HOSHIZAKI』として、世界No.1の食のインフラづくりに挑んでいます。ここでは、本社を構成するそれぞれの職種の役割と仕事内容をご紹介します。」 |
| J-4 | 職種紹介 / Job ダイアグラム | `.p-job__diagram` | 3 つのボックスフロー：**製品を生み出す**（開発・設計／生産管理・購買／生産技術／品質保証）／**製品を広める**（国内販売促進／海外事業企画）／**会社を管理・サポートする**（コーポレート（総務、経理、人事、他）／社内 SE）。ボックス間は矢印「連携」「サポート」で接続 |
...
| 語学研修制度 | 社員の主体的な学びを後押しするため、語学研修制度を設けています。海外売上比率50％を超えるグローバル企業として、世界を舞台に活躍するために必要な語学力の習得を支援しています。（※Figma 原文は「資格取得補助制度を設けています」と別制度の説明になっていたため、こちらで作成した差し替え案。要クライアント承認） |
...
| 名称 | 補足 |
|:--|:--|
| 福利厚生サービス | 多種サービスを優待価格で利用できる会員制の福利厚生です。 |
...
| R-2 | リード | `.p-requirement__lead` | 「ホシザキはフードサービス機器の枠を超え、病院や農業、流通など新たな分野への挑戦を続けています。食材ロスや環境負荷といった社会課題の解決を通じ、世界中の『食』の未来を創ることが私たちの使命です。現状に満足せず、グローバルな舞台で『常識の先へ』跳躍したいと願う、情熱ある皆さんのエントリーをお待ちしています。」 |
...
| 待遇・福利厚生・社内制度 | 福利厚生サービス、退職金制度、財形貯蓄制度、社員持株会制度、資格取得補助制度、独身寮制度（入寮要件あり）、社宅制度、会社保養施設（山荘）、クラブ活動補助（野球、バスケット、テニス、卓球、茶華道 等）、慶弔見舞金、食事補助 他 |
...
| # | セクション | クラス | 主な中身 |
|:--|:--|:--|:--|
| X-1 | ヒーロー | `.p-special-story__hero` | 会議中の 4 人の写真（幅いっぱい）／左下にラベル「クロストーク」＋白い大見出し「若手社員が語るはたらく環境」 |
| X-2 | 導入 | `.p-special-story__intro` | 「常識を飛び越え、高め合う。入社4年目が語るホシザキの『リアル』と『チームワーク』。…」（4〜5 行） |
...
1. 就活時は「フードサービス機器のメーカー」というイメージだったと思います。実際に入社してみて、仕事内容や働く環境にギャップはありましたか？
...
| I-9 | 採用担当からのメッセージ / MESSAGE | `.p-internship__message` | 見出し「採用担当からのメッセージ／MESSAGE」／本文（中央寄せ）：「『ホシザキを、ただのフードサービス機器メーカーと思っていないか。』私たちが求めているのは、既存の枠を超え、新しい『食の当たり前』を創り出すパイオニア精神です。世界中の厨房、そして病院や農業、流通の現場を支える製品が、どのような思考と情熱から生まれるのか。この5日間で、あなたの『常識』を塗り替える体験が待っています。」／「さぁ、その想いを翼に変えて。エントリーはマイページから。」 |
