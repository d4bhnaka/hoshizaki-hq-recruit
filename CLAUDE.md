...
- **Relative paths everywhere** — `astro.config.mjs` sets `base: "./"` so the site works from any subdirectory. The root page references `./css/style.css`; subpages reference `../css/style.css`. `Layout.astro` accepts a `basePath` prop (default `"./"`) — **every page under `src/pages/` that isn't the index must pass `basePath="../"`** (or deeper) to the layout. See [src/pages/message.astro](src/pages/message.astro) and [src/layouts/Layout.astro](src/layouts/Layout.astro). (Two-level pages such as [src/pages/person/[slug].astro](src/pages/person/) and [src/pages/special/crosstalk.astro](src/pages/special/crosstalk.astro) pass `basePath="../../"`.)
